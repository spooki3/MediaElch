...
{
    return QStringLiteral("Universal Music Scraper");
}
...
{
    QUrl url(QStringLiteral("https://musicbrainz.org/ws/2/artist/?query=artist:\"%1\"")
...
void UniversalMusicScraper::onSearchArtistFinished()
{
    ...
    if (reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt() == 302
        ...
        reply = m_qnam.get(QNetworkRequest(reply->attribute(QNetworkRequest::RedirectionTargetAttribute).toUrl()));
        new NetworkReplyWatcher(this, reply);
        connect(reply, &QNetworkReply::finished, this, &UniversalMusicScraper::onSearchArtistFinished);
        return;
    }
...
{
    auto reply = static_cast<QNetworkReply *const>(QObject::sender());
    Artist *const artist = reply->property("storage").value<Storage *>()->artist();
    ...
    appendDownloadElement(artist,
        "theaudiodb",
        "tadb_data",
        QUrl(QStringLiteral("http://www.theaudiodb.com/api/v1/json/%1/artist-mb.php?i=%2")
                 .arg(m_tadbApiKey)
                 .arg(artist->mbId())));
    appendDownloadElement(artist,
        "theaudiodb",
        "tadb_discography",
        QUrl(QStringLiteral("http://www.theaudiodb.com/api/v1/json/%1/discography-mb.php?s=%2")
                 .arg(m_tadbApiKey)
                 .arg(artist->mbId())));
...
void UniversalMusicScraper::onArtistLoadFinished()
{
    ...
    if (!artist || !m_artistDownloads.contains(artist)) {
        return;
    }

    auto artistItemToDownload = std::find_if(m_artistDownloads[artist].begin(),
        m_artistDownloads[artist].end(),
        [&reply](const auto &download) { return download.url == reply->url(); });

    if (artistItemToDownload == m_artistDownloads[artist].end()) {
        return;
    }

    if (reply->error() == QNetworkReply::NoError) {
        artistItemToDownload->contents = QString::fromUtf8(reply->readAll());
    } else {
        qWarning() << "Network Error (load)" << reply->errorString();
    }
    artistItemToDownload->downloaded = true;
...
bool UniversalMusicScraper::isArtistDownloaded(Artist &artist) const
{
    const auto downloadedIterator = std::find_if(m_artistDownloads[&artist].cbegin(),
        m_artistDownloads[&artist].cend(),
        [](const auto &download) { return !download.downloaded; });
    return downloadedIterator == m_artistDownloads[&artist].cend();
}

bool UniversalMusicScraper::shouldLoad(int info, QList<int> infos, Album *album) const
{
...
bool UniversalMusicScraper::shouldLoad(int info, QList<int> infos, Artist *artist) const
{
    if (!infos.contains(info)) {
        return false;
    }
...
bool UniversalMusicScraper::infosLeft(QList<int> infos, Album *album) const
{
...
bool UniversalMusicScraper::infosLeft(QList<int> infos, Artist *artist) const
{
    foreach (int info, infos) {
